...
ignore_dirs = {".venv", ".git", "cmake-build-debug", "cmake-build-debug-event-trace", "tooling", "JetBrainsPlugins",
               r"the_ui_tree_build\__pycache__", ".idea", r"the_code\pybind11"}
...
for root, dirs, files in os.walk(folder):
    ...
    for f_name in files:
            ...
            with f_path.open("r", encoding="utf-8", errors="ignore") as file:
                for line in file:
                    ...
                    if in_multiline:
                        ...
                        in_multiline = any(
                            end not in s or s.find(start) > s.find(end) for start, end in markers.get("multi", []))
                        continue
        ...
        stats_per_lang[f_path.suffix]["loc"] = stats_per_lang[f_path.suffix]["total"] - stats_per_lang[f_path.suffix][
            "empty"] - stats_per_lang[f_path.suffix]["full_comment"]
