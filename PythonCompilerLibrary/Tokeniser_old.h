    ...
    std::vector<token> tokenize(const std::string& input_text){
        ...
        while (token_pos < text.size()){
            ...
            if (token_literal.sucses){
                if (token_literal.token.type == TokenType::DOT){
                    char c = peek_token();
                    if (std::isdigit(c)){
                        token tok = number_parse(true);
...
private:
    ...
    token number_parse(const bool& is_float){
        token tok;
        bool is_float_internal = is_float;
        ...
        if (number_literal_value.back() == '_'){
            throw std::runtime_error("Unterminated number");
        }
